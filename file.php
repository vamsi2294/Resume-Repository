...
<?php 
	...
	$uploadOk = 1;
    $sql = "INSERT INTO candidates ". "(FirstName,LastName, type, email, State, PhnNumber,FileName,FileSize,FileType,FilePath,resume,rating,notes) ". "VALUES('$fname','$lname','$type','$email','$state','$pnum','$name','$size','$type',' $path','$content','$rating','$notes')";
    if ($conn->query($sql) === TRUE) {
   	  echo "New record created successfully";
	} else {
  	  ?>
    <div align="center" class="alert alert-danger col-md-offset-4 col-md-5 ">
    <strong >Email already uploaded</strong>
    </div>
    <?php
	}
	    ...
	    <?php
			if ($result->num_rows > 0) {
			    while($row = $result->fetch_assoc()) {
		            ...
		            echo "<td><a href='viewfile.php?name=".$row["email"]."' target=_blank >View</a>
